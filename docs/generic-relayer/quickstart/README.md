...
Welcome to the ZK Email Generic Relayer Quickstart Guide! This guide will walk you through setting up and using the Generic Relayer to enable email-driven actions on blockchain networks. You'll learn how to define commands, deploy contracts, and interact with the relayer API.
...
```bash
mkdir generic-relayer && cd generic-relayer
```
...
Since we're using Foundry, we need to install certain dependencies via `forge`.
